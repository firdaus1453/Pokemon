saya ingin menambahkan endpoint baru untuk mendapatkan data dari API https://pokeapi.co/api/v2/pokemon?limit=3&offset=0, dengan response :
...
saya juga ingin menggabungkan data detail dari Pokemon contoh dari api ini https://pokeapi.co/api/v2/pokemon/1/, untuk bisa menampilkan di list home sesuai dengan kebutuhan UI yang sudah saya buat pada HomeScreen, kamu harus baca kebutuhan UI terlebih dahulu HomeScreen lihat juga kebutuhan yang ada pada UI HomeDetailScreen

Saya ingin juga kita bisa membuat database dengan Room untuk data pokemon yang sudah kita fetch pada halaman Home, agar kita bisa mengaksesnya secara offline

Buat juga agar untuk menampilkan nama-nama pokemon sebanyak 10 dan bisa di update ketika di scroll ke bawah. Ketika di listnya di klik masuk ke halaman detail

Ikuti pattern yang sudah saya buat, dan saya menggunakan retrofit, kamu tinggal buat HttpClientFactory dan lain-lainnya, baca dulu code saya sebelumnya

response api terkait detail pokemon seperti ini, tolong kamu analisa dan sesuaikan dengan kebutuhan UI kita, kalau datanya tidak ada tidak perlu kita tampilkan
Ini contoh response nya :
{
"id": 35,
"name": "clefairy",
"base_experience": 113,
